# app.py (FLAT - no folders) - Trento EV Charging Investment Tool
...
from demand import (
    demand_from_parking_model,
    demand_from_funnel_model,
)
from sizing import (
    suggest_mix_from_targets,
    compute_capacity_kwh_per_year,
)
from finance import (
    build_cashflows,
    npv,
...
from optimizer import (
    optimize_mix_bruteforce,
)

st.set_page_config(
    page_title="Trento EV Charging Investment Tool",
    page_icon="⚡",
    layout="wide",
)
...
mode = st.sidebar.radio(
    "Modalità domanda",
    ["Data-driven (CSV parcheggio)", "Funnel (stima macro)"],
    index=0,
)
...
st.caption("Valuta ROI/NPV/IRR, CAPEX/OPEX e strategia dimensionamento (AC fino 22kW, DC fino 120kW).")
...
with tabs[0]:
    ...
    with c1:
        st.markdown("### Colonnine esistenti (Comune di Trento / upload CSV)")
        st.write("Puoi caricare un CSV di colonnine (anche estratto da fonti pubbliche) per avere un riepilogo competitivo.")
        chargers_file = st.file_uploader("Upload CSV colonnine (opzionale)", type=["csv"])
        chargers_url = st.text_input("oppure URL CSV colonnine (opzionale)", value="")
        ...
        if chargers_file is not None:
            ...
            try:
                ...
                st.warning(f"Impossibile scaricare CSV da URL: {e}")

        if chargers_df is not None:
            summary = summarize_trento_chargers(chargers_df)
    ...
    with c2:
        st.markdown("### Dati parcheggio (CSV) - per modalità data-driven")
        st.write(
            "Carica un CSV con colonna tempo e una metrica: "
            "`occupancy/occupazione/occupied` oppure `free/liberi/posti_liberi`."
        )
        parking_file = st.file_uploader("Upload CSV parcheggio (opzionale)", type=["csv"])
        st.caption("Esempio: timestamp, occupancy (0..1 o 0..100 o posti occupati)")
...
if "parking_df" not in st.session_state:
    st.session_state["parking_df"] = None
if "chargers_df" not in st.session_state:
    st.session_state["chargers_df"] = None

# update session state from tab 1 variables if exist
try:
    if "parking_df" in locals() and parking_df is not None:
        st.session_state["parking_df"] = parking_df
except Exception:
    pass
try:
    if "chargers_df" in locals() and chargers_df is not None:
        st.session_state["chargers_df"] = chargers_df
except Exception:
    pass
...
with tabs[1]:
    ...
    if mode == "Data-driven (CSV parcheggio)":
        st.markdown("### Modello data-driven (parcheggio → traffico → BEV → sessioni → kWh)")
        if st.session_state["parking_df"] is None:
            st.info("Carica un CSV parcheggio nel tab **1) Dati** per usare questa modalità.")
        else:
            ...
            daily_traffic = estimate_daily_traffic_from_parking(
            ...
            demand = demand_from_parking_model(
                ...
                share_dc=0.25,  # baseline split, can be refined in sizing tab
                days_per_year=365,
            )

            st.metric("Traffico stimato (veicoli/giorno)", f"{daily_traffic:,.0f}".replace(",", "."))
            st.metric("Domanda annua stimata (kWh/anno)", f"{demand['kwh_year']:,.0f}".replace(",", "."))
            st.write("Dettaglio stima:")
            st.json(demand)

            st.session_state["demand_kwh_year"] = demand["kwh_year"]

    else:
        st.markdown("### Modello funnel (macro)")
        c1, c2, c3 = st.columns(3)
        with c1:
        ...
        demand = demand_from_funnel_model(
            bev_count=bev_count,
        ...
        st.metric("Domanda annua stimata (kWh/anno)", f"{demand['kwh_year']:,.0f}".replace(",", "."))
        st.write("Dettaglio funnel:")
        st.json(demand)

        st.session_state["demand_kwh_year"] = demand["kwh_year"]

    if "demand_kwh_year" not in st.session_state:
        st.session_state["demand_kwh_year"] = 0.0
...
with tabs[2]:
    ...
    if demand_kwh_year <= 0:
        ...
        st.markdown("### Risultato sizing (minimo per coprire domanda con utilizzo target)")
        colA, colB, colC = st.columns(3)
        colA.metric("Punti AC suggeriti", f"{sizing['n_ac']}")
        colB.metric("Punti DC suggeriti", f"{sizing['n_dc']}")
        ...
        st.write(
            f"- Capacità AC: **{cap_ac:,.0f} kWh/anno**\n"
            f"- Capacità DC: **{cap_dc:,.0f} kWh/anno**\n"
            f"- Totale: **{(cap_ac+cap_dc):,.0f} kWh/anno**"
        )
...
with tabs[3]:
    ...
    if demand_kwh_year <= 0 or cfg is None:
        st.info("Completa **2) Domanda** e **3) Sizing & Mix** prima di vedere il business case.")
    else:
        c1, c2, c3 = st.columns(3)
        with c1:
        ...
        capex_total = (
            cfg["n_ac"] * cfg["ac_capex"]
            + cfg["n_dc"] * cfg["dc_capex"]
            + fixed_capex
        )
        ...
        kpis = st.columns(4)
        kpis[0].metric("CAPEX totale", format_eur(capex_total))
        kpis[1].metric("NPV", format_eur(npv_val))
        kpis[2].metric("IRR", f"{(irr_val*100):.1f}%" if irr_val is not None else "n/a")
        kpis[3].metric("Payback (anni)", f"{pb:.1f}" if pb is not None else "n/a")

        st.markdown("### Cashflow annuo")
        df = cash["table"]
        st.dataframe(df, use_container_width=True)

        st.download_button(
            "Scarica cashflow CSV",
...
with tabs[4]:
    ...
    if demand_kwh_year <= 0:
        ...
        with c6:
            share_dc = st.slider("Quota domanda su DC (%)", 0, 80, 25, 1, key="opt_share_dc") / 100.0
        ...
        if st.button("Esegui ottimizzazione"):
            ...
            if res is None:
                ...
                st.write("**Risultato**")
                st.json(res["best"])
                st.markdown("### Top risultati (anteprima)")
                st.dataframe(res["top_table"], use_container_width=True)
                st.download_button(
                    "Scarica top risultati CSV",
                    data=res["top_table"].to_csv(index=False).encode("utf-8"),
                    file_name="optimizer_top_results.csv",
                    mime="text/csv",
                )
